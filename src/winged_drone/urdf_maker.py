...
class UrdfMaker:
    ...
    def __init__(self, prm: Union[GeometryParams, Sequence[float]], *, out_dir: Union[str, Path] = "urdf_generated") -> None:
        if not isinstance(prm, GeometryParams):
            ...
            chord = span / ar
            seq[1] = chord
            prm = GeometryParams(*seq)  # type: ignore[arg-type]
        self.p = prm
    ...
    def _scale(self, what: str, dims: Tuple[float, float, float]) -> str:
        ...
        rd = {
            "wing":     (self._REF["tw"],  self._REF["wing_chord"], dims[2]),
    ...
    def create_urdf(self, filename: str | None = None) -> Path:
        ...
        """
        if filename is None:
            filename = self._params_as_string() + ".urdf"
...
if __name__ == "__main__":
    ref1 = [0.70, 0.2, -0.275, -0.275, 0.73, 0.18, 0.14, 0.16, 0.12, 1, 0, 0.10, 0.14, 2, 2.5, 2, -3]
    ref2 = [0.50, 0.15, -0.275, -0.275, 0.6, 0.14, 0.12, 0.12, 0.10, 1, 0, 0.10, 0.14, 2, 2.5, 2, -3]
    print("URDF written to", UrdfMaker(ref1).create_urdf())
