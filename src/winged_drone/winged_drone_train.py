...
def train(exp_name, urdf_file, num_envs, max_iterations, parent_exp: str | None = None, parent_ckpt: int | None = None):
    ...
    env_cfg["wide_depth_critic"] = False
    ...
    if parent_exp is not None and parent_ckpt is not None:
        parent_dir = os.path.join("logs", parent_exp)
        ckpt_path  = os.path.join(parent_dir, f"model_{parent_ckpt}.pt")
...
def main():
    ...
    # Ottieni le configurazioni per l'environment e per il training
    env_cfg, obs_cfg, reward_cfg, command_cfg = get_cfgs()
    env_cfg["wide_depth_critic"] = args.wide_depth_critic
