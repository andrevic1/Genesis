...
from morphing_drone.aerodyn_solver import compute_aero_details as morph_physics
...
class WingedDroneEnv:
       ...
       # camera
        ...
        if self.drone_name == "morphing_drone":
            self.drone = self.scene.add_entity(gs.morphs.URDF(
                file=env_cfg.get("urdf","urdf/mydrone/mydrone.urdf"),
        ...
        self.num_obs      = (
            ...
            + self.num_commands         # heading, z_target
        )
        ...
        self.drone.set_dofs_kp(torch.full((self.num_servos,),500.0,device=self.device), self.servo_dof_indices)
        self.drone.set_dofs_kv(torch.full((self.num_servos,),  20.0,device=self.device), self.servo_dof_indices)
    ...
    def _generate_forests(self):
        """
        Crea le foreste.
        - In *evaluation* resta la vecchia griglia rettangolare.
        - In *non-evaluation* gli alberi sono posizionati nell'anello
        20 m ≤ r ≤ 80 m (centro in (0,0)).
        """
        num_trees   = self.env_cfg.get("num_trees", 100)
        num_trees_eval = self.env_cfg.get("num_trees_eval", 100)
        tree_radius = self.env_cfg.get("tree_radius", 1.0)
        tree_height = self.env_cfg.get("tree_height", 50.0)

        # -------- quante foreste generare (identico a prima) ---------------------
        if self.evaluation:
        ...
        # --------- coordinate cilindri ------------------------------------------
        if self.evaluation:
            # === nuovo metodo: anello radiale ====================================
            r    = gs_rand_float(30.0, 200.0, (total_forests, num_trees*2), self.device)
            phi  = gs_rand_float(-math.pi, math.pi, (total_forests, num_trees*2), self.device)
            xs   = r * torch.cos(phi)
            ys   = r * torch.sin(phi)
        else:
            # === nuovo metodo: anello radiale ====================================
            r    = gs_rand_float(30.0, 100.0, (total_forests, num_trees), self.device)
            phi  = gs_rand_float(-math.pi, math.pi, (total_forests, num_trees), self.device)
            xs   = r * torch.cos(phi)
            ys   = r * torch.sin(phi)

        if self.evaluation:
            cyl   = torch.zeros((total_forests, num_trees*2, 3),
                                dtype=torch.float16 if (not self.evaluation or self.unique_forests_eval) else torch.float32,
                                device=self.device)
        else:
            cyl   = torch.zeros((total_forests, num_trees, 3),
                                dtype=torch.float16 if (not self.evaluation or self.unique_forests_eval) else torch.float32,
                                device=self.device)
        cyl[..., 0] = xs
        cyl[..., 1] = ys
        cyl[..., 2].fill_(tree_height * 0.5)
        self.cylinders_array = cyl

        # Disegno gli alberi della prima foresta solo se serve visualizzare
        if self.evaluation and not self.unique_forests_eval:
            for i in range(num_trees):
                self.scene.add_entity(gs.morphs.Cylinder(
                    pos=cyl[0, i, :].cpu().numpy(),
                    radius=tree_radius,
                    height=tree_height,
                    collision=False,
                    fixed=True,
    ...
    def _resample_commands(self, envs_idx):
        """
        Estrae comandi random:
        angle   ~ U(-π, π)
        z_target~ U(min_z_target, max_z_target)
        """
        if self.evaluation:
            min_z = self.command_cfg.get("min_z_target", 20.0)
            max_z = self.command_cfg.get("max_z_target", 20.0)
            angles = gs_rand_float(-math.pi/3, math.pi/3, (len(envs_idx),), self.device)
        else:
            min_z = self.command_cfg.get("min_z_target", 15.0)
            max_z = self.command_cfg.get("max_z_target", 25.0)
            angles = gs_rand_float(-math.pi/3, math.pi/3, (len(envs_idx),), self.device)

        z_tgt  = gs_rand_float(min_z,     max_z,  (len(envs_idx),), self.device)

        self.commands[envs_idx, 0] = angles          # heading
    ...
    def step(self, actions):
        ...
        self.drone.control_dofs_position(exec_actions[:, 1:], self.servo_dof_indices)

        if self.drone_name == "morphing_drone":
            aero_details = morph_physics(self.drone, exec_actions[:,0])
            ...
            F, M, alpha, beta = bix3_physics(self.drone, exec_actions[:,0], max_thrust=15)
            self.alpha = alpha
            self.beta = beta
            force = F.unsqueeze(1)
        ...
        if self.env_cfg.get("aero_noise", True):
            ...
            abs_ang = (torch.abs(self.alpha) + torch.abs(self.beta))        # (E,) deg                             # → rad
        ...
        self.obs_buf = torch.cat(
            [
                (self.base_pos[:, 2].unsqueeze(1) - 20)/20,
                self.base_quat,
                self.base_lin_vel[:, 0].unsqueeze(1)/10,
                ...
                (self.commands[:, 1].unsqueeze(1) - 20)/10,
            ],
            axis=-1,
        )
    ...
    def check_collision(self, tol=0.05):
        ...
        """
        half_span = self.env_cfg.get("wing_span",        1.6) * 0.5  # m
    ...
    def check_success(self):
        """
        Successo se il modulo (x,y) ≥ 80 m.
        In evaluation continua a valere la vecchia soglia lineare (forest_x_limit)
        per retro-compatibilità.
        """
        if self.evaluation:
            radius = torch.norm(self.base_pos[:, :2], dim=1)
            return radius >= 200.0

        # non-evaluation – anello
        radius = torch.norm(self.base_pos[:, :2], dim=1)
        return radius >= 100.0
    ...
    def reset_idx(self, envs_idx):
        ...
        speed0 = self.env_cfg.get("init_speed", 9.0)
        speed = torch.full((num_resets,), speed0, device=self.device, dtype=gs.tc_float)
        angles = self.commands[envs_idx, 0]                   # rad
        if not self.evaluation:
            noise = gs_rand_normal(0.0, 2.0, (num_resets,), self.device)
            speed += noise
        self.base_pos[envs_idx]       = torch.tensor([0.0, 0.0, 20.0], device=self.device)
        # velocità linearmente nella direzione voluta
        self.base_lin_vel[envs_idx, 0] = speed * torch.cos(angles)
        self.base_lin_vel[envs_idx, 1] = speed * torch.sin(angles)
        self.base_lin_vel[envs_idx, 2] = 0.0

        # assetto: roll, pitch = 0; yaw = angle
        self.base_euler[envs_idx] = torch.stack((
            torch.zeros_like(angles),             # roll
            torch.zeros_like(angles),             # pitch
            angles                                # yaw
        ), dim=1)

        if not self.evaluation:
            noise = gs_rand_normal(0.0, 3.0, (num_resets,), self.device)
            self.base_pos[envs_idx, 2] += noise
            noise = gs_rand_normal(0.0, 0.05, (num_resets, len(self.servo_dof_indices)), self.device)
            self.joint_position[envs_idx] += noise
            noise = gs_rand_normal(0.0, 0.0, (num_resets,), self.device)
        ...
        if self.evaluation and not self.unique_forests_eval:
            n_dofs = self.rigid_solver.get_dofs_velocity().shape[1]
            # 2. vettore pieno inizializzato a zero
            full_vel = torch.zeros((num_resets, n_dofs),
                                device=self.device, dtype=gs.tc_float)
            # 3. riempi le parti che controlli
            full_vel[:, 0:3] = self.base_lin_vel[envs_idx]   # xyż
            full_vel[:, 3:6] = self.base_ang_vel[envs_idx]   # ω
            for j, dof in enumerate(self.servo_dof_indices):
                full_vel[:, dof] = self.joint_velocity[envs_idx, j]
            # 4. invia al solver
            self.rigid_solver.set_dofs_velocity(full_vel, envs_idx=envs_idx)

            full_pos = torch.zeros((num_resets, n_dofs),
                                device=self.device, dtype=gs.tc_float)

            # xyz + RPY
            full_pos[:, 0:3] = self.base_pos[envs_idx]
            full_pos[:, 3:6] = self.base_euler[envs_idx]

            # servo joint positions
            for j, dof in enumerate(self.servo_dof_indices):
                full_pos[:, dof] = self.joint_position[envs_idx, j]

            # (tutti gli altri DOF – elica inclusa – restano a zero)
            self.rigid_solver.set_dofs_position(full_pos, envs_idx=envs_idx)
    ...
    def _reward_progress(self):
        """Reward = metri avanzati lungo la direzione comando, diviso dt."""
        # spostamento nell'ultimo step (world frame)
        disp = self.base_pos[:, :2] - self.last_base_pos[:, :2]      # (E,2) [m]

        # versore della direzione richiesta
        angles = self.commands[:, 0]                                 # (E,)
        d_unit = torch.stack((torch.cos(angles), torch.sin(angles)), dim=1)  # (E,2)

        # proiezione scalare (dot-product) => metri avanzati lungo d_unit
        prog = torch.sum(disp * d_unit, dim=1)                       # (E,) [m]

        # trasformo in m/s (coerente con altri reward legati alla velocità)
        return prog / self.dt
                                # (E,)
    ...
    def _reward_height(self):
        """
        Penalità quadratica sull’errore di quota rispetto al comando.
        """
        height = torch.square(self.base_pos[:, 2] - self.commands[:, 1])
        return height
